...
async function serializeTempo(
  ...
  // TIP-76: sender does not commit to feeToken. Strip it for both the
  // sign payload (no `signature`) and the partial relay-handoff envelope
  // (no `feePayerSignature`). Keep it only on the final broadcast
  // envelope so the chain can verify the fee payer.
